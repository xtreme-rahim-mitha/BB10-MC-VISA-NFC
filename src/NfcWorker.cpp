...
void NfcWorker::doIso7816Test(const QVariant &aid, const QVariant &hex_cla, const QVariant &hex_ins, const QVariant &hexp1p2, const QVariant &hex_lc,
	...
	for (i = 0; i < numberOfReaders; i++) {
		...
		emit message(QString("SE reader name: ") + QString(readerName));
		if ((len == 3) && (strcmp(readerName, "SIM") == 0)) {
			...
			emit message(QString("we found non-sim reader"));
	}
...
void NfcWorker::handleNfcReadNdefTagEvent(bps_event_t *event) {
	...
	if (NFC_TAG_READWRITE_EVENT == code) {
		...
		for (unsigned int ndefMsgIndex = 0; ndefMsgIndex < ndefMsgCount; ++ndefMsgIndex) {
			...
			emit message(QString("Custom tag written OK2 "+ QString(ndefMsgIndex)));
			nfc_ndef_message_t *ndefMessage;
...
//	}
	...
	qDebug() << "XXXX NfcWorker::handleNfcReadNdefTagEvent - NDEF read event";
			...
			qDebug() << "XXXX NfcWorker::handleNfcReadNdefTagEvent - got NFC event object";
			if(nfcEvent)
				emit message(QString("Retrieving Event... " ));

			nfc_target_t* target = NULL;
			CHECK(nfc_get_target(nfcEvent, &target));
			if(target)
				emit message(QString("Retrieving Target..." ));

			char buffer[5];
			nfc_get_tag_property(target,TAG_PROP_ISO_14443_4_TYPE, buffer,5);

			qDebug() << "XXXX NfcWorker::handleNfcReadNdefTagEvent - got target";
			char name[10];
			size_t name_length;
			nfc_get_tag_name(target, name, 10, &name_length);
			qDebug() << "XXXX NfcWorker::handleNfcReadNdefTagEvent - got target name: " << name;


			nfc_create_ndef_message (&myNdefMessage);

			nfc_ndef_record_t* record = 0;
			QString type = QString("text/x-vCard");
			QString text = QString("00A404000e315041592e5359532e444446303100");//command
			int textLen = text.length();
			int totalLen = textLen;
			uchar_t payload[totalLen];
			int offset = 0;
			memcpy(&payload[offset], text.toUtf8().constData(), textLen);
			nfc_create_ndef_record(NDEF_TNF_WELL_KNOWN, type.toUtf8().constData(),
			...
			nfc_result_t rct = nfc_write_ndef_message_to_tag(target, myNdefMessage, false);



			///
//			 	 	 	 	 	 	   00A404000e315041592e5359532e4444 46 30 31 00
			                                     //1 PAY.SYS.DDF01
//			//const uchar_t command[] = "00A404000E322E5041592E5359532E4444463031";
//			const uchar_t command[] = "00A4040007A0000000031010";
//			//const uchar_t command[] = "00A4040007A0000000031010";
//			//const uchar_t command[] = "00A404080411122";
//			//0020008008246666FFFFFFFFFF
//			size_t command_length = 12;
//			uchar_t response[1024]={0};
//			size_t response_length = 0;
//			nfc_result_t rct = nfc_tag_transceive(target, TAG_TYPE_ISO_14443_4,
//			                                        command,
//			                                        command_length,
//			                                        response,
//			                                        1024,
//			                                        &response_length);
			...
			qDebug() << "XXXX NfcWorker::handleNfcReadNdefTagEvent - target message count=" << ndefMsgCount;
			if(ndefMsgCount > 0)
				emit message(QString("Custom tag written OK: 1 " ));
			else
				emit message(QString("Custom tag written OK: 0 ") );
			for (unsigned int ndefMsgIndex = 0; ndefMsgIndex < ndefMsgCount; ++ndefMsgIndex) {
				...
				nfc_ndef_message_t *ndefMessage;
				CHECK(nfc_get_ndef_message(target, ndefMsgIndex, &ndefMessage));
				emit message(QString("Custom tag written OK: 2 "));
				parseNdefMessage(ndefMessage);
				CHECK(nfc_destroy_target(target));
			}
	}
	else
		emit message(QString("Target Status Code: " ) + QString::number(code));
}
...
void NfcWorker::parseNdefMessage(nfc_ndef_message_t *ndefMessage) {
	...
	ndefMsgCount = 1;
	emit message(QString("Custom tag written OK3 "));
	for (int ndefMsgIndex = 0; ndefMsgIndex < ndefMsgCount; ++ndefMsgIndex) {
		...
		for (unsigned int ndefRecordIndex = 0; ndefRecordIndex < ndefRecordCount; ++ndefRecordIndex) {
			emit message(QString("Custom tag written OK4 "));
